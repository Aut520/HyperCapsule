...
internal fun IosLiquidGlassNavigationBar(
    ...
    // The visible row owns tap targets. The backdrop sampling row and the
    // moving indicator are visual duplicates; making their children clickable
    // would create a sibling hit path and starve the drag detector on the
    // visible row (the exact reason long-press dragging used to stop at a flash).
    val tabsContent: @Composable RowScope.(Boolean) -> Unit = { interactive ->
            ...
            CompositionLocalProvider(LocalContentColor provides tabContentColor) {
                Row(
                    modifier = Modifier
                        ...
                        // KernelSU keeps one drag detector on the stable outer row. The
                        // moving indicator and the transparent sampling row remain visual
                        // layers, so a long-press can continue while the pill moves under it.
